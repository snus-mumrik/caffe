...
void ConvolutionShareBoostLayer<Dtype>::SetUp(const vector<Blob<Dtype>*>& bottom,
      vector<Blob<Dtype>*>* top) {
  Layer<Dtype>::SetUp(bottom, top);
  ConvolutionParameter conv_param = this->layer_param_.convolution_share_boost_param().convolution_param();
  ...
  bool random_test = this->layer_param_.convolution_share_boost_param().share_boost_param().random_test();
  num_iterations_per_round_ = this->layer_param_.convolution_share_boost_param().share_boost_param().num_iterations_per_round();
  n_passes_ = 0;
  elements_per_feature_ = this->layer_param_.convolution_share_boost_param().share_boost_param().num_elements_per_feature();
  CHECK_EQ(elements_per_feature_, 0) << "Can only activate whole channels in convolution share boost";
  elements_per_feature_ = kernel_h_ * kernel_w_;
  num_input_features_ = (channels_ / group_);
  max_num_of_rounds_ = this->layer_param_.convolution_share_boost_param().share_boost_param().max_num_of_rounds();
  ...
  if (this->blobs_.size() > 0) {
    ...
    caffe_set(num_output_*(channels_/group_)*kernel_h_*kernel_w_, Dtype(0), this->blobs_[0]->mutable_cpu_data());
    weigth_fill_.Reshape(num_output_, channels_ / group_, kernel_h_, kernel_w_);
    // fill the weights
    ...
    if (random_test) {
      ...
      for (int idx_output = 0; idx_output < num_output_; idx_output++) {
	for (int k = 0; k < num_active_features_; k++) {
	  for (int edx = 0; edx < elements_per_feature_; edx++) {
	    weights[idx_output*K_ + k*elements_per_feature_ + edx] = weigth_fill[idx_output*K_ + k*elements_per_feature_ + edx]; // TODO mem copy
	  }
	}
      }      
    }
...
template <typename Dtype>
Dtype ConvolutionShareBoostLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
      vector<Blob<Dtype>*>* top) {
...
template <typename Dtype>
void ConvolutionShareBoostLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
  ...
  if ((num_active_features_ == 0) || 
	...
	if (active_features_.cpu_data()[k])
	  continue;
	Dtype cur_L1 = 0.0;
	for (int n = 0; n < num_output_; n++) { // No need to iterate over images in batch because weigths_diff is calculated for the whole batch
	  for (int edx = 0; edx < elements_per_feature_; edx++) {
  ...
  Dtype* weights_diff = weight_diff;
...
#ifdef CPU_ONLY
STUB_GPU(ConvolutionShareBoostLayer);
#endif

INSTANTIATE_CLASS(ConvolutionShareBoostLayer);
